...
public class AXE {
	...
	private static void injectIntoFrames(final WebDriver driver, final String script,
		...
		for (WebElement frame : frames) {
			try {
				driver.switchTo().defaultContent();
				if (parents == null) {
					throw new NullPointerException("String value parents is null");
				}
				if (parents != null) {
	...
	public static void writeResults(final String name, final Object output) {
		try(Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(name + ".json"), "utf-8"))) {
	...
	/**
	 * Chainable builder for invoking aXe. Instantiate a new Builder and configure
	 * testing with the include(), exclude(), and options() methods before calling
	 * analyze() to run.
	 */
